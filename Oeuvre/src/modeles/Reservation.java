...
public class Reservation {
	 ...
	 * 
	 * @param id_oeuvre
	 *            Id de l'oeuvre réservée
	 * @param id_adherent
	 *            Id de l'adhérent réservant
	 * @throws Exception
	 */
	...
	public void Lire(int id) throws Exception {
		...
		try {
			...
			ps = connection
					.prepareStatement("Select * from reservation where id_oeuvrevente="
							+ id);
			rs = ps.executeQuery();
			while (rs.next()) {
				...
				this.setDate_reservation(rs.getDate("date_reservation"));
			}
	...
	public void Modifier() throws Exception {
		...
		try {
			...
			String mysql = "UPDATE reservation SET statut ='Confirmee' WHERE id_oeuvrevente ="
					+ this.getId_oeuvre() + ";";
	...
	public void Ajouter() throws Exception {
		...
		try {
			...
			String mysql = "INSERT INTO reservation VALUES ('"
					+ this.getOeuvre().getId_oeuvre() + "','"
					+ this.getAdherent().getId_adherent() + "','Attente',\'"
					+ dd + "\')";
	...
	public void supprimerReservation(int num_oeuvre) throws Exception {
		PreparedStatement ps = null;
		...
		try {
			...
			String mysql = "DELETE FROM reservation WHERE id_oeuvrevente ="
					+ num_oeuvre + ";";
			ps = connection.prepareStatement(mysql);

			ps.execute();

			String mysql1 = "UPDATE oeuvrevente set etat_oeuvrevente ='L' WHERE id_oeuvrevente ="
					+ num_oeuvre + ";";

			ps = connection.prepareStatement(mysql1);
